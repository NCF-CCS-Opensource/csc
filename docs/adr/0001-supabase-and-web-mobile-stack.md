...
Status: accepted, with the auth portion withdrawn. Supabase is now a **Postgres and Storage host only** — auth moved to Clerk (ADR-0012, superseding ADR-0003, which had superseded the magic-link choice here). Supabase was chosen here largely because its auth mapped onto our email-verification requirement; that justification no longer applies. Drizzle, Turborepo, Vercel, Expo, and Resend are unaffected.
