...
`students`, `programs` (seeded with the 4 defaults), `semesters`, `events`, `attendance_sessions`, `scans`, and `penalties` exist so far. Re-run `db:generate` after schema changes and `db:migrate` to apply.
