...
**Report**:
An on-demand PDF document generated by an Officer or Governor, containing structured attendance/financial data and an AI-generated narrative analysis. Four types exist: per-Event (available the day after the Event date), per-Student (a Student's Ledger for one Semester), per-Semester (aggregate summary), and financial (penalties charged vs. collected). The AI narrative uses only anonymized aggregate data; student PII is rendered directly by the PDF layout and never sent to the AI provider. Reports are not stored — each generation is fresh.
_Avoid_: Analytics, summary
