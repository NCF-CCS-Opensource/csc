...
import { flushQueue } from "./syncScans";
...
describe("flushQueue", () => {
  it("delivers all 50 queued decisions in capture order while retaining only five Recent scans", async () => {
    ...
    expect((await loadQueue()).map(({ officerId }) => officerId)).toEqual(["officer-b"]);
    expect(await loadRecentScans("officer-a")).toHaveLength(5);
    expect((await loadRecentScans("officer-a")).every(({ deliveryState }) => deliveryState === "synced")).toBe(true);
  });
  ...
  it("marks permanent failures Failed and continues with later decisions", async () => {
    ...
    expect(await loadQueue("officer-a")).toEqual([
      { ...queued("1"), deliveryState: "failed", error: "Unknown student" },
    ]);
    expect(await loadRecentScans("officer-a")).toEqual([
      { ...recent("2"), deliveryState: "synced" },
    ...
    await flushQueue("officer-a");
    expect(await loadQueue("officer-a")).toEqual([]);
    expect((await loadRecentScans("officer-a"))[1].deliveryState).toBe("synced");
  });
});
