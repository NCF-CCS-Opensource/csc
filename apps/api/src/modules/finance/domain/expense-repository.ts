...
export interface ExpenseRepository {
  ...
  // needs — including the Semester it was recorded against, for the per-Semester
  // spending breakdown (issue #348). Voided rows are kept in; computeDepartmentFund
  // excludes them, so the Expense (money-out) void rule sits in the domain rather
  // than the query. (Money-in void exclusion lives upstream in computeFinancialReport,
  // which the Fund reuses — the two streams filter voids in their own layers.)
  listForFund(): Promise<{ amount: string; voidedAt: Date | null; semesterId: string }[]>;
