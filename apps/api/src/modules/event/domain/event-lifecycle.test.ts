...
import { deriveWholeDayPenalty, validateEventInput, validateEventUpdate } from "./event-lifecycle";

// Ported from apps/web/lib/events.test.ts.
describe("validateEventInput", () => {
  ...
  it("rejects an invalid type", () => {
    expect(
      validateEventInput({ ...valid, type: "quarter_day" as never }, semesterRange),
    ).toEqual([{ field: "type", message: "Select whole-day or half-day" }]);
  });
  ...
  it("rejects a non-numeric penalty amount", () => {
    expect(
      validateEventInput({ ...valid, halfDayPenaltyAmount: "abc" }, semesterRange),
    ).toEqual([{ field: "halfDayPenaltyAmount", message: "Penalty amount must be greater than 0" }]);
  });
