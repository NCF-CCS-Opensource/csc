...
export async function GET(
  ...
  const officerRows =
    officerIds.length > 0
      ? await db
          ...
          .from(students)
          .where(inArray(students.id, officerIds))
      : [];
  ...
  const reportData = computeFinancialReport({
    semester: {
      id: semester.id,
      name: semesterName,
  ...
  const pdfBuffer = await renderToBuffer(
    FinancialPdfDocument({ data: reportData }) as any,
  );

  const filename = `financial-report-${semester.startDate}-to-${semester.endDate}.pdf`;
